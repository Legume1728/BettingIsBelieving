...
def random_demo(p=0.5):
    ...
    d={};
    d['period'] = 0;
    names= ["alarm", "dog_barks", "dog_eats", "earth_quake"];
    d['betting_var']=choice(names);
    #shuffle(names); #randomly assign labels
    #n1, n2, n3, n4 = names;

    d=indepVar("alarm", p, d)
    d=indepVar("earth_quake", p, d)
    #d=one_cause(n2, n1, p, d)
    d=causes("dog_eats", ["alarm", "earth_quake"], d, p,  set(["alarm", "earth_quake"]))
    d=causes("dog_barks", ["alarm", "earth_quake", "dog_eats"], d, p, set(["dog_eats"]))
    return d
...
def random_intermediate(p=0.5):
    ...
    d={}
    d['period']=0;
    names=["interest_rate", "industry", "financial_sector", "consumer_spending"];
    d['betting_var']=choice(names);
    d=indepVar("interest_rate", p, d)
    d=indepVar("industry", p, d)
    d=one_cause("financial_sector", "interest_rate", p, d)
    d=causes("consumer_spending", ["interest_rate", "industry"], d, p, neg=set(["interest_rate"]))
    return d

def random_simple(p=0.5):
    d={}
    d['period']=0;
    names=["interest_rate", "financial_sector", "industry"];
    d['betting_var']=choice(names);
    d=indepVar("interest_rate", p, d)
    d=indepVar("financial_sector", p, d)
    d=causes("industry", ["interest_rate", "financial_sector"], d, p, neg=set(["interest_rate"]))
    return d

def random_complex(p=0.5):
    d={}
    d['period']=0;
    names=["interest_rate", "financial_sector", "industry", "consumer_spending"];
    d['betting_var']=choice(names);
    d=indepVar("interest_rate", p, d);
    d=indepVar("financial_sector", p, d);
    d=causes("industry", ["interest_rate", "financial_sector"], d, p, neg=set(["interest_rate"]))
