                ...
                curKeys.map(function(key)
                    {
                        ...
                        {
			    if (predictionCanvas.tags[key].isRendered()){
				predictionCanvas.tags[key].erase();
			    }
                            if (typeof(predictionCanvas.tags[key].rect)!=="undefined")
                            {
				if (predictionCanvas.tags[key].rect.isRendered()){
...
{
    var betButton = makeButton("Buy", 90, 25, "green");
    betButton.xPos=xPos;
    betButton.callback = {
        //'betView':betView,
        'truth':truth,
    ...
    //betView.pointsInp.registerEnterCallback(callback);
    makeClickable(betButton, betButton.callback, "green");
    return betButton;
}
    
function makeSellButton(xPos)
{
    var putButton = makeButton("Sell", 90, 25, "red");
    putButton.xPos=xPos;
    var callback = {
        //'betView': betView,
        'truth': truth,
    ...
    //betView.pointsInp.registerEnterCallback(callback);
    makeClickable(putButton, callback, "red");
    return putButton;
}
